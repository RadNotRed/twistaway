...
Twistaway separates authentication from private data encryption.
...
Routes, home address, ride logs, and account-associated network logs are treated as
sensitive.
...
The backend stores encrypted payload envelopes and operational metadata required to
serve the account. Plain usernames and emails are allowed by the product requirements.

## Backend Logs

Anonymous operational logs should be minimized and rotated. Authenticated user audit
logs should be encrypted before storage. If the server must generate a user-associated
log value such as an observed IP address, the client should establish a short-lived
audit encryption session after login so the server can encrypt that event without
storing the user's raw password.
